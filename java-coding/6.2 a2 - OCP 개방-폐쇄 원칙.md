...
따라서 이런 경우, [[6.1 c - interface]]를 사용한다
...
SRP([[6.2 a1 - SRP 단일 책임 원칙]]) → 수직적 구분
...
이에 따라서 단일 책임 원칙은 지켰지만 개방-폐쇄 원칙은 지키지 못한 코드도 발생하게 된다
...
여기서 또 `meterToCentimeter`가 추가되면 `AreaConvertor` 클래스를 수정해야된다.

또한, 아까처럼 `List`로 일괄적 관리를 위해선 마찬가지로 또다시 `if-else()` 구문을 꺼내야된다

따라서 이또한 [[6.1 c - interface]]로 구현하겨나, 혹은 `AreaConvertor`를 추상클래스로 만들고, 이를 상속받게 하는게 좋은 코드가 된다
